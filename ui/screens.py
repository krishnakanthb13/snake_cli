...
class ScreenManager:
    ...
    def show_start_page(self):
        ...
        menu_items = [
            ...
            ("Snake Color", "snake_color", ["GREEN", "BLUE", "RED", "YELLOW", "WHITE", "CYAN", "MAGENTA"]),
            ("Mode", "snake_mode", ["same_color", "mixed_color", "mixed_shape", "mixed_both"]),
            ("Food Color", "food_color", ["RANDOM", "YELLOW", "CYAN", "MAGENTA", "WHITE", "RED", "BLUE", "GREEN"]),
            ("Big Food %", "big_food_chance", [10, 20, 30, 50]),
        ...
        while True:
            ...
            settings_count = len(menu_items) - 3
            for i, item in enumerate(menu_items):
                label, key_name, options = item
                text = f"{label}"
                if options:
                ...
                if i < settings_count:
                    if i == 0:
                        self.renderer.draw_text(5, 0, "--- SETTINGS ---", curses.A_DIM, center=True)
                    row = 7 + i # Gap after header (line 6 is blank)
                else:
                    if i == settings_count:
                        self.renderer.draw_text(7 + settings_count + 1, 0, "--- ACTIONS ---", curses.A_DIM, center=True)
                    row = 7 + settings_count + 3 + (i - settings_count) # Gap after header
                
                attr = curses.A_REVERSE if i == current_selection else 0
                self.renderer.draw_text(row, 0, text, attr, center=True)
            
            self.renderer.draw_text(self.renderer.max_y - 2, 0, "Use Arrows to Navigate | Space/Enter to Select", curses.A_DIM, center=True)
            ...
            if key in [curses.KEY_UP, ord('w')]:
                ...
                current_selection = (current_selection + 1) % len(menu_items)
            elif key in [10, 13, 459, curses.KEY_ENTER, ord(' ')]: # Enter (Standard/Numpad) or Space
                ...
                if key_name == "start":
                    ...
                    self.config_manager.set(key_name, new_val)
                    
                    # If we just changed the theme, apply its color immediately
                    if key_name == "theme":
                        if new_val in THEMES:
                            self.config_manager.set("snake_color", THEMES[new_val]["snake"])
            elif key == ord('q'):
                return "quit"

    def show_pause_overlay(self):
        y, x = self.renderer.max_y // 2, self.renderer.physical_x // 2
    ...
    def show_end_page(self, state):
        ...
        curses.echo()
        curses.curs_set(1)
        name_input = self.renderer.stdscr.getstr(12, (self.renderer.physical_x - 10) // 2, 15).decode('utf-8')
        curses.noecho()
        curses.curs_set(0)
        ...
        walls = "ON" if settings.get("walls_on") else "OFF"
        mode = settings.get("mode", "?")
        shape = settings.get("shape", "?")
        
        while True:
            self.renderer.clear()
            self.renderer.draw_text(3, 0, "G A M E   O V E R", curses.A_BOLD, center=True)
            self.renderer.draw_text(4, 0, "=================", 0, center=True)
            
            # Stats section
            self.renderer.draw_text(6, 0, "--- FINAL STATS ---", curses.A_DIM, center=True)
            ...
            self.renderer.draw_text(9, 0, f"Snake Length: {state.snake_length}", 0, center=True)
            self.renderer.draw_text(10, 0, f"Small Food: {state.food_eaten['small']} | Big Food: {state.food_eaten['big']}", 0, center=True)
            
            # Settings section
            self.renderer.draw_text(12, 0, "--- SETTINGS USED ---", curses.A_DIM, center=True)
            self.renderer.draw_text(13, 0, f"Speed: {speed} | Walls: {walls} | Shape: {shape}", 0, center=True)
            self.renderer.draw_text(14, 0, f"Mode: {mode}", 0, center=True)
            
            # Actions
            self.renderer.draw_text(17, 0, "[ R ] Play Again", 0, center=True)
            self.renderer.draw_text(18, 0, "[ M ] Main Menu", 0, center=True)
            self.renderer.draw_text(19, 0, "[ H ] View History", 0, center=True)
            self.renderer.draw_text(20, 0, "[ Q ] Exit", 0, center=True)
            self.renderer.refresh()
    ...
    def show_leaderboard(self):
        ...
        while True:
            ...
            self.renderer.draw_text(self.renderer.max_y - 2, 0, "[S] Toggle Sort | [Any] Return", curses.A_DIM, center=True)
            self.renderer.refresh()
            
            key = self.renderer.get_input()
            if key in [ord('s'), ord('S')]:
                self.leaderboard_manager.toggle_sort()
            else:
                break
    ...
    def show_stats_overlay(self, state):
        ...
        h, w = 10, 40
        y, x = (self.renderer.max_y - h) // 2, (self.renderer.physical_x - w) // 2
    ...
    def show_history(self):
        """Show all past game sessions with pagination."""
        self.renderer.stdscr.nodelay(False)
        all_sessions = self.stats_manager.get_all_sessions()
        page = 0
        per_page = self.renderer.max_y - 10
        
        while True:
            self.renderer.clear()
            self.renderer.draw_text(2, 0, "G A M E   H I S T O R Y", curses.A_BOLD, center=True)
            self.renderer.draw_text(3, 0, "=======================", 0, center=True)
            
            if not all_sessions:
                self.renderer.draw_text(6, 0, "No games played yet!", 0, center=True)
            else:
                total_pages = (len(all_sessions) - 1) // per_page + 1
                start_idx = page * per_page
                end_idx = min(start_idx + per_page, len(all_sessions))
                
                self.renderer.draw_text(5, 0, f"Page {page + 1} of {total_pages} ({len(all_sessions)} total games)", curses.A_DIM, center=True)
                
                row = 7
                for session in all_sessions[start_idx:end_idx]:
                    player = session.get("player", "Unknown")[:10]  # Truncate long names
                    score = session.get("score", 0)
                    time_s = session.get("time_survived", 0)
                    timestamp = session.get("timestamp", "")[:10]  # Just the date
                    self.renderer.draw_text(row, 0, f"{timestamp} | {player:<10} | Score: {score:>5} | Time: {time_s:>4}s", 0, center=True)
                    row += 1
            
            self.renderer.draw_text(self.renderer.max_y - 2, 0, "[</>] Prev/Next Page | [Q] Return", curses.A_DIM, center=True)
            self.renderer.refresh()
            ...
            if key in [ord('q'), ord('Q'), 27]:  # Q or Escape
                ...
                if all_sessions and (page + 1) * per_page < len(all_sessions):
                    page += 1
